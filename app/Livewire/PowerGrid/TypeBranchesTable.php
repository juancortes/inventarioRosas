...
final class TypeBranchesTable extends PowerGridComponent
{
...
//            Filter::datepicker('created_at_formatted', 'created_at'),
    ...
    public function actions(\App\Models\TypeBranches $row): array
    {
        return [
            Button::make('show', file_get_contents('assets/svg/eye.svg'))
//                ->slot('Show')
                ->class('btn btn-outline-info btn-icon w-100')
                ->tooltip('Mostrar Detalles de Tipo de Ramo')
                ->route('typeBranches.show', ['typeBranches' => $row])typeBranches
                ->method('get'),

            Button::make('edit', file_get_contents('assets/svg/edit.svg'))
                ->class('btn btn-outline-warning btn-icon w-100')
                ->route('typeBranchess.edit', ['typeBranches' => $row])
                ->method('get')
                ->tooltip('Editar Tipo de Ramo'),

            Button::add('delete')
                ->slot(file_get_contents('assets/svg/trash.svg'))
                ->class('btn btn-outline-danger btn-icon w-100')
                ->tooltip('Borrar Tipo de Ramo')
                ->route('typeBranches.destroy', ['typeBranches' => $row])
                ->method('delete'),
        ];
    }
}
